 ...
 */
 ...
 struct combo_cfg {
     ...
     /* Optional array defining key positions that cancel the combo if pressed first. */
     int32_t key_positions_first_exclude[MAX_COMBO_KEYS];
     int16_t key_position_len_first_exclude;
     ...
     COND_CODE_1(                                                                                   \
         ...
         (                                                                                          \
             {                                                                                      \
                 ...
                 .key_positions_first_exclude = DT_PROP_OR(n, key_positions_first_exclude, {}),     \
                 .key_position_len_first_exclude = DT_PROP_LEN_OR(n, key_positions_first_exclude, 0), \
 ...
 static int setup_candidates_for_first_keypress(int32_t position, int64_t timestamp) {
     ...
     for (size_t i = 0; i < ARRAY_SIZE(combos); i++) {
         if (sys_bitfield_test_bit((mem_addr_t)&combo_lookup[position], i)) {
             ...
             bool first_key_allowed = true;
             if (combo->key_position_len_first_exclude > 0) {
                 for (int idx = 0; idx < combo->key_position_len_first_exclude; idx++) {
                     if (position == combo->key_positions_first_exclude[idx]) {
                         first_key_allowed = false;
                         break;
                     }
                 }
             }
 
             if (leader_ok && first_key_allowed && combo_active_on_layer(combo, highest_active_layer) &&
